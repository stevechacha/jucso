from django.urls import path
from rest_framework_simplejwt.views import TokenRefreshView

from core import views

urlpatterns = [
    path("health/", views.HealthView.as_view(), name="health"),
    path("auth/login/", views.LoginView.as_view(), name="login"),
    path("auth/register/", views.StudentRegisterView.as_view(), name="register"),
    path("auth/token/refresh/", TokenRefreshView.as_view(), name="token-refresh"),
    path("auth/password-reset/", views.PasswordResetRequestView.as_view(), name="password-reset"),
    path("auth/password-reset/confirm/", views.PasswordResetConfirmView.as_view(), name="password-reset-confirm"),
    path("auth/me/", views.MeView.as_view(), name="me"),
    path("auth/change-password/", views.ChangePasswordView.as_view(), name="change-password"),
    path("auth/verify-email/", views.EmailVerifyView.as_view(), name="verify-email"),
    path("auth/resend-verification/", views.ResendVerificationView.as_view(), name="resend-verification"),
    path("complaints/", views.ComplaintListCreateView.as_view(), name="complaint-list"),
    path("complaints/categories/", views.ComplaintCategoriesView.as_view(), name="complaint-categories"),
    path("complaints/track/", views.ComplaintTrackView.as_view(), name="complaint-track"),
    path("complaints/<str:tracking_id>/", views.ComplaintDetailView.as_view(), name="complaint-detail"),
    path("complaints/<str:tracking_id>/rate/", views.ComplaintRateView.as_view(), name="complaint-rate"),
    path("complaints/<str:tracking_id>/escalate/", views.ComplaintEscalateView.as_view(), name="complaint-escalate"),
    path("complaints/<str:tracking_id>/de-escalate/", views.ComplaintDeEscalateView.as_view(), name="complaint-de-escalate"),
    path("suggestions/", views.SuggestionListCreateView.as_view(), name="suggestion-list"),
    path("suggestions/<int:pk>/", views.SuggestionDetailView.as_view(), name="suggestion-detail"),
    path("clubs/", views.ClubListView.as_view(), name="club-list"),
    path("clubs/<int:pk>/join/", views.ClubJoinView.as_view(), name="club-join"),
    path("events/", views.EventListView.as_view(), name="event-list"),
    path("events/calendar.ics", views.EventsCalendarView.as_view(), name="events-calendar"),
    path("events/<int:pk>/register/", views.EventRegisterView.as_view(), name="event-register"),
    path("elections/", views.ElectionListView.as_view(), name="election-list"),
    path("elections/<int:pk>/vote/", views.ElectionVoteView.as_view(), name="election-vote"),
    path("news/", views.NewsListView.as_view(), name="news-list"),
    path("news/<str:news_id>/", views.NewsDetailView.as_view(), name="news-detail"),
    path("announcement/", views.ActiveAnnouncementView.as_view(), name="active-announcement"),
    path("notifications/", views.NotificationListView.as_view(), name="notifications"),
    path("notifications/mark-read/", views.NotificationMarkReadView.as_view(), name="notifications-mark-read"),
    path("push/vapid-public-key/", views.PushVapidPublicKeyView.as_view(), name="push-vapid-public-key"),
    path("push/subscribe/", views.PushSubscribeView.as_view(), name="push-subscribe"),
    path("documents/", views.DocumentListView.as_view(), name="document-list"),
    path("contact/", views.ContactCreateView.as_view(), name="contact"),
    path("leadership/", views.LeadershipListView.as_view(), name="leadership-list"),
    path("stats/public/", views.PublicStatsView.as_view(), name="public-stats"),
    path("stats/executive/", views.ExecutiveStatsView.as_view(), name="executive-stats"),
    path("stats/minister-workload/", views.MinisterWorkloadView.as_view(), name="minister-workload"),
    path("stats/transparency/", views.TransparencyStatsView.as_view(), name="transparency-stats"),
    path("admin/users/", views.AdminUsersView.as_view(), name="admin-users"),
    path("admin/users/<path:reg_number>/", views.AdminUserUpdateView.as_view(), name="admin-user-update"),
    path("admin/staff/", views.AdminStaffCreateView.as_view(), name="admin-staff-create"),
    path("admin/ministries/", views.MinistryListView.as_view(), name="ministry-list"),
    path("admin/documents/", views.AdminDocumentCreateView.as_view(), name="admin-document-create"),
    path("admin/documents/<int:pk>/", views.AdminDocumentDetailView.as_view(), name="admin-document-detail"),
    path("admin/news/", views.AdminNewsCreateView.as_view(), name="admin-news-create"),
    path("admin/news/<int:pk>/", views.AdminNewsDetailView.as_view(), name="admin-news-detail"),
    path("admin/announcements/", views.AdminAnnouncementListCreateView.as_view(), name="admin-announcements"),
    path("admin/announcements/<int:pk>/", views.AdminAnnouncementDetailView.as_view(), name="admin-announcement-detail"),
    path("admin/contact-messages/", views.AdminContactMessageListView.as_view(), name="admin-contact-messages"),
    path("admin/contact-messages/mark-all-read/", views.AdminContactMessageMarkAllReadView.as_view(), name="admin-contact-mark-all-read"),
    path("admin/contact-messages/bulk-delete/", views.AdminContactMessageBulkDeleteView.as_view(), name="admin-contact-bulk-delete"),
    path("admin/contact-messages/<int:pk>/", views.AdminContactMessageDetailView.as_view(), name="admin-contact-message-detail"),
    path("admin/clubs/", views.AdminClubCreateView.as_view(), name="admin-club-create"),
    path("admin/clubs/<int:pk>/", views.AdminClubDetailView.as_view(), name="admin-club-detail"),
    path("admin/clubs/<int:pk>/members/", views.AdminClubMembersView.as_view(), name="admin-club-members"),
    path("admin/events/", views.AdminEventCreateView.as_view(), name="admin-event-create"),
    path("admin/events/<int:pk>/", views.AdminEventDetailView.as_view(), name="admin-event-detail"),
    path("admin/events/<int:pk>/registrants/", views.AdminEventRegistrantsView.as_view(), name="admin-event-registrants"),
    path("admin/backup/", views.AdminBackupView.as_view(), name="admin-backup"),
    path("admin/backup/restore/", views.AdminBackupRestoreView.as_view(), name="admin-backup-restore"),
    path("admin/elections/", views.AdminElectionCreateView.as_view(), name="admin-election-create"),
    path("admin/audit-log/", views.AdminAuditLogListView.as_view(), name="admin-audit-log"),
    path("admin/contact-messages/<int:pk>/reply/", views.AdminContactMessageReplyView.as_view(), name="admin-contact-message-reply"),
    path("admin/system-status/", views.AdminSystemStatusView.as_view(), name="admin-system-status"),
    path("admin/overview/", views.AdminOverviewView.as_view(), name="admin-overview"),
]
